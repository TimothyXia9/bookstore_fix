...
class CategorySecond(models.Model):
    ...
    up = models.ForeignKey(
        CategoryFirst, related_name="down", on_delete=models.CASCADE, verbose_name="上级"
    )
...
class Book(models.Model):
    ...
    discount = models.DecimalField(max_digits=2, decimal_places=1, verbose_name="折扣")
    ...
    def get_recommend_hot_books(current_category=None):
        ...
        if current_category:
            recommend_hot_books = current_category.books.order_by("-sales").all()[:50]
        else:
    ...
    def get_new_books(current_category=None):
        if current_category:
            new_books = current_category.books.order_by("-added_time").all()[:10]
        else:
...
class Comment(models.Model):
    book = models.ForeignKey(
        Book, related_name="comments", on_delete=models.CASCADE, verbose_name="图书"
    )
